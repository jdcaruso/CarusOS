...
// Forward declaration
static void create_app_window(int app_id, lv_obj_t * return_screen);
static void create_app_icon(lv_obj_t * parent, const char * icon, const char * text, int app_id);
...
static void create_app_window(int app_id, lv_obj_t * return_screen) {
    ...
    switch(app_id) {
        ...
        case 3: { // Settings App (Folder Metaphor)
            ...
            create_app_icon(app_grid, LV_SYMBOL_BELL, "NTP Sync", 1);
            create_app_icon(app_grid, LV_SYMBOL_AUDIO, "Audio", 2);
            create_app_icon(app_grid, LV_SYMBOL_FILE, "Sys Info", 5); // Sys Info Stats
#if CARUSOS_APP_EXPLORER
            create_app_icon(app_grid, LV_SYMBOL_DIRECTORY, "Archivos", 7); // File Explorer
#endif
            create_app_icon(app_grid, LV_SYMBOL_DOWNLOAD, "Update", 6); // OTA Update
            create_app_icon(app_grid, LV_SYMBOL_SETTINGS, "Opciones", 8); // Options
            create_app_icon(app_grid, LV_SYMBOL_POWER, TXT_SCREEN_SLEEP, 4); // Special ID for sleep
            break;
        }
        case 5: { // Sys Info Stats Window
        ...
        case 6: { // System Update (OTA) Window
...
#if CARUSOS_APP_GALLERY
        case 9: { // Gallery Web App
...
static void app_launcher_event_cb(lv_event_t * e) {
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        if (app_id == 4) {
            // Instant Sleep (no window)
            lvgl_port_set_backlight(false);
        } else {
            // Open window and pass current screen as return screen
            create_app_window(app_id, lv_scr_act());
        }
    }
}

// Helper to create App Icons
static void create_app_icon(lv_obj_t * parent, const char * icon, const char * text, int app_id) {
    ...
    // Grey out Audio app if audio is disabled
#if !CARUSOS_USE_AUDIO
    if (app_id == 2) {
    ...
    // Grey out OTA app if OTA is disabled
#if !CARUSOS_USE_OTA
    if (app_id == 6) {
        lv_obj_set_style_text_color(icon_label, lv_color_hex(0x555555), 0);
        lv_obj_set_style_text_color(text_label, lv_color_hex(0x555555), 0);
...
static void create_main_screen() {
    ...
    // Populate Apps (Only Calculator and Config for Desktop)
    create_app_icon(app_grid, LV_SYMBOL_PLUS, TXT_APP_CALC_TITLE, 0);
#if CARUSOS_APP_GALLERY
    create_app_icon(app_grid, LV_SYMBOL_IMAGE, "Galeria", 9); // Web Gallery
#endif
    create_app_icon(app_grid, LV_SYMBOL_SETTINGS, TXT_APP_INFO_TITLE, 3);
}
